import { useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../contexts/AuthContext';
import './LoginPage.css';

// API URL injected at build time
declare const __API_BASE_URL__: string;

export default function LoginPage() {
  const { isAuthenticated, loading } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    if (isAuthenticated) {
      navigate('/dashboard');
    }
  }, [isAuthenticated, navigate]);

  if (loading) {
    return (
      <div className="login-page">
        <div className="login-container">
          <div className="loading">Loading...</div>
        </div>
      </div>
    );
  }

  return (
    <div className="login-page">
      <div className="login-container">
        <div className="login-header">
          <h1>🤖 TeamAI</h1>
          <p className="tagline">Scale Your Agency with AI Agents</p>
        </div>

        <div className="login-content">
          <h2>Welcome Back</h2>
          <p className="subtitle">Sign in with your Google account to continue</p>

          <a 
            href={`${__API_BASE_URL__.replace(/\/api$/, '')}/api/v1/auth/google/login`}
            className="google-login-btn"
          >
            <svg className="google-icon" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Sign in with Google
          </a>

          <div className="login-features">
            <h3>Why TeamAI?</h3>
            <ul>
              <li>🚀 Deploy AI agents instantly</li>
              <li>👥 Manage teams and assignments</li>
              <li>📊 Track performance and usage</li>
              <li>🔐 Enterprise-grade security</li>
            </ul>
          </div>
        </div>

        <div className="login-footer">
          <p>Don't have access? Contact your agency administrator.</p>
        </div>
      </div>
    </div>
  );
}
